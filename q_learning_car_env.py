...
class GameState:
    ...
    def frame_step(self, action):
        ...
        for shape in self.space.shapes:
            ...
            elif isinstance(shape, pymunk.Circle):  # Check if the shape is a circle
                body = shape.body
                pos = int(body.position.x), int(height - body.position.y)  # Convert pymunk coordinates to pygame
                if body.body_type == 0:
...
if __name__ == "__main__":
    game_state = GameState(random_env=False)
    while True:
